...
const getHeatmapColor = (
  ...
  return interpolateColor(colorScale[segmentIndex], colorScale[segmentIndex + 1], segmentProgress)
}
      ...
      <Svg width={chartWidth} height={height}>
        {uniqueRows.map((row, rowIndex) =>
          uniqueCols.map((col, colIndex) => {
            ...
            const cellColor = getHeatmapColor(value, minValue, maxValue, colorScale)
              ...
              <G key={`cell-${row}-${col}`}>
                ...
                {showLabels && cellWidth > 30 && cellHeight > 20 && (
                  <SvgText
                    ...
                    fill={value > (minValue + maxValue) / 2 ? "#ffffff" : textColor}
                    fontWeight="500"
                  >
          ...
          <G>
            {uniqueRows.map((row, rowIndex) => (
              <SvgText
                key={`row-label-${row}`}
                x={padding - 8}
                y={padding + rowIndex * (cellHeight + cellSpacing) + cellHeight / 2 + 4}
                textAnchor="end"
                fontSize={12}
                fill={mutedColor}
              >
        ...
        {showLabels && (
          <G>
            {uniqueCols.map((col, colIndex) => (
              <SvgText
                key={`col-label-${col}`}
                x={padding + colIndex * (cellWidth + cellSpacing) + cellWidth / 2}
                y={height - 5}
                textAnchor="middle"
                fontSize={12}
                fill={mutedColor}
