...
const squarify = (
  ...
  while (remainingData.length > 0) {
    ...
    for (let i = 1; i <= remainingData.length; i++) {
      ...
      const worstRatio = Math.max(
        ...row.map((item) => {
          ...
          return Math.max(rowDimension / itemDimension, itemDimension / rowDimension)
        }),
      )
    ...
    const rowValue = bestRow.reduce((sum, item) => sum + item.normalizedValue, 0)
    const rowDimension = rowValue / dimension
...
export const TreeMapChart = ({ data, config = {}, style }: Props) => {
  ...
  const { height = 200, padding = 10, showLabels = true, animated = true, duration = 800 } = config
