...
const BaseStack = React.forwardRef<View, InternalProps>(function BaseStack(
  ...
  const dir = reverse ? (__direction === "row" ? "row-reverse" : "column-reverse") : __direction

  const resolvedFlex: number | undefined = typeof flex === "boolean" ? (flex ? 1 : undefined) : flex
  ...
  // Normalize children
  const kids = React.Children.toArray(children).filter((c) => c !== null && c !== undefined && !!c)
...
export const XStack = React.forwardRef<View, XStackProps>(function XStack(props, ref) {
  return <BaseStack ref={ref} __direction="row" {...props} />
})

export const YStack = React.forwardRef<View, YStackProps>(function YStack(props, ref) {
