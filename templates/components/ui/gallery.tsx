...
export const useImageZoom = ({
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      // Constrain translation
      const constrained = constrainTranslation(scale.value, newTranslateX, newTranslateY)
        ...
        {(showTitles || showDescriptions) && (
          <View style={styles.itemInfo}>
            {showTitles && item.title && (
              <Text variant="subtitle" numberOfLines={1} style={{ color: textColor }}>
                {item.title}
              </Text>
            )}
...
const styles = StyleSheet.create({
  bottomControls: {
    ...
    paddingBottom: 46,
    position: "absolute",
    right: 0, // Adjust for safe area (home indicator)
  },
  container: {
    flex: 1,
  },
  ...
  thumbnailContainer: {
    alignItems: "center",
    paddingHorizontal: 16, // Vertically center thumbnails
  },
  thumbnailImage: {
    height: "100%",
    width: "100%",
