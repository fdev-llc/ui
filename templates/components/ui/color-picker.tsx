...
export const ColorSwatch: React.FC<ColorSwatchProps> = ({ color, size = 32, style, onPress }) => {
...
export const ColorPicker: React.FC<ColorPickerProps> = ({
  ...
  const hueGesture = Gesture.Pan().onUpdate((event) => {
    const newX = Math.max(0, Math.min(PICKER_SIZE - KNOB_SIZE, event.x - KNOB_SIZE / 2))
  ...
  const pickerGesture = Gesture.Pan().onUpdate((event) => {
    const newX = Math.max(0, Math.min(PICKER_SIZE - KNOB_SIZE, event.x - KNOB_SIZE / 2))
    const newY = Math.max(0, Math.min(PICKER_SIZE - KNOB_SIZE, event.y - KNOB_SIZE / 2))
      ...
      >
        <View style={[styles.modalContainer, { backgroundColor: card }]}>
          ...
          <View style={styles.content}>
            ...
            >
              <Text variant="caption" style={{ color: textColor, fontWeight: "bold" }}>
              ...
              <GestureDetector gesture={hueGesture}>
                <Animated.View>
                  <Svg width={PICKER_SIZE} height={HUE_BAR_HEIGHT}>
                    ...
                    <Rect width={PICKER_SIZE} height={HUE_BAR_HEIGHT} fill="url(#hue)" rx={10} />
                  </Svg>
