...
export function ComboboxTrigger({ children, style, error = false }: ComboboxTriggerProps) {
...
export function ComboboxValue({ placeholder = "Select...", style }: ComboboxValueProps) {
...
export function ComboboxContent({ children, maxHeight = 400 }: ComboboxContentProps) {
...
export function ComboboxInput({
  ...
  return (
    <View style={[styles.searchContainer, { borderBottomColor: borderColor }, style]}>
      <TextInput
        style={withGeistFont([styles.searchInput, { color: textColor }])}
    ...
    if (isValidElement(child) && child.type === ComboboxGroup) {
      ...
      return groupChildren.some((groupChild) => {
        if (isValidElement(groupChild) && groupChild.type === ComboboxItem) {
          ...
          const searchText = itemProps.searchValue || label || itemProps.value || ""
          return searchText.toLowerCase().includes(searchQuery.toLowerCase())
        }
        return false
      })
  ...
  return (
    <View style={[styles.emptyContainer, style]}>
      {typeof children === "string" ? (
        <Text style={[styles.emptyText, { color: mutedColor }]}>{children}</Text>
      ) : (
        children
      )}
    </View>
...
export function ComboboxGroup({ children, heading }: ComboboxGroupProps) {
  ...
  return (
    <View>
      {heading && <Text style={[styles.groupHeading, { color: mutedColor }]}>{heading}</Text>}
      {filteredChildren}
    </View>
  )
}
