...
export function PopoverContent({
  ...
  const getPosition = () => {
    ...
    const contentHeight = Math.min(contentSize.height || maxHeight, screenDimensions.height * 0.8)
    ...
    } else if (side === "bottom" && top + contentHeight > screenDimensions.height - padding) {
    ...
    } else if (side === "right" && left + contentWidth > screenDimensions.width - padding) {
...
export function PopoverHeader({ children, style }: PopoverHeaderProps) {
  ...
  return <View style={[styles.header, { borderBottomColor: borderColor }, style]}>{children}</View>
}
...
export function PopoverFooter({ children, style }: PopoverFooterProps) {
  ...
  return <View style={[styles.footer, { borderTopColor: borderColor }, style]}>{children}</View>
}
...
export function PopoverClose({ children, asChild = false, style }: PopoverCloseProps) {
...
const styles = StyleSheet.create({
  ...
  content: {
    ...
    shadowOpacity: 0.25,
    shadowRadius: 6, // Ensure minimum width
  },
  footer: {
    borderTopWidth: 1,
    flexDirection: "row",
