...
export function ShareButton({
  ...
  const handleShare = useCallback(async () => {
    if (!isContentValid) {
      const error = new Error("Invalid share content: At least one of message or url is required")
      onShareError?.(error)
    ...
    try {
      ...
      const shareError = error instanceof Error ? error : new Error(String(error))
  ...
  const buttonContent = () => {
    ...
    const textContent =
      typeof children === "string" ? <Text style={getButtonTextStyle()}>{children}</Text> : children
...
export function useShare() {
  const shareText = useCallback((text: string, options?: ShareButtonOptions) => {
    return RNShare.share({ message: text }, options)
  }, [])

  const shareUrl = useCallback((url: string, message?: string, options?: ShareButtonOptions) => {
    return RNShare.share({ url, message }, options)
  }, [])

  const shareContent = useCallback((content: ShareContent, options?: ShareButtonOptions) => {
    const shareData: any = {}
